...
class VisionTransformer:
    ...
    def get_features(self, images: Optional[torch.Tensor] = None, texts: Optional[torch.Tensor] = None, audios: Optional[torch.Tensor] = None) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        return (
            outputs["vision"].detach().cpu().float().numpy() if images is not None else None, 
            outputs["text"].detach().cpu().float().numpy() if texts is not None else None, 
            outputs["audio"].detach().cpu().float().numpy() if audios is not None else None
        )
