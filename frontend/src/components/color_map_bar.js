...
const Color_map_bar = ({cmap, set_cmap, clusters, scores, max_index}) => {
    ...
    const render_color_map_bar = (cmap) => {
        if (cmap === "default") {
            ...
            let color_stops = [MAGMA1, MAGMA2, MAGMA3, MAGMA4, MAGMA5, MAGMA6];
            render_color_map_bar_gradient(0, max_index, color_stops, "white", "black");
        }
            ...
            let color_stops = [LOW_SCORE_COLOR, VIRIDIS1, HIGH_SCORE_COLOR];
            render_color_map_bar_gradient(min_value, max_value, color_stops, "black", "black");
        }
    }
     ...
     * @param {*} color_stops expected array of objects with red, green and blue fields, 
     * gradient colors from first to last*/
    const render_color_map_bar_gradient = (min_value, max_value, color_stops, text_color1, text_color2) => {
        const color_map_bar = color_map_bar_ref.current;
        const ctx = color_map_bar.getContext("2d", {alpha: true});
        const width = color_map_bar.width;
        const height = color_map_bar.height;
        ...
        for (let i = 0; i < color_stops.length; i++) {
            gradient.addColorStop(i / (color_stops.length - 1),
                `rgb(${color_stops[i].red}, ${color_stops[i].green}, ${color_stops[i].blue})`);
        } 
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, width, height);
    
        // Add min and max values
        ctx.fillStyle = text_color1;
        ctx.font = '18px Arial';
    ...
    return (
        <div className="row h-5 w-100">
            <input 
                type="button" 
                value="default" 
                className="col-2 h-100 btn btn-primary responsive-text"
                onClick={handle_cmap_buttons_on_click}
            />
            <input 
                type="button" 
                value="clusters" 
                className="col-2 h-100 btn btn-secondary responsive-text"
                onClick={handle_cmap_buttons_on_click}
            />
            <input 
                type="button" 
            ...
            />
            <canvas 
                className="col-4 h-100 d-block rounded p-0"
